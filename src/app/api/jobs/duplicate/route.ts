...
export async function POST(req: NextRequest) {
  ...
  const data = body as { job_id?: string };
  ...
  try {
    const response = await callN8n<JobActionResponse>("/webhook/sa-job-duplicate", {
      job_id,
      requesterEmail: session.user.email
    });
    return NextResponse.json(response);
    const data = await callN8n<{ ok: boolean; job: NormalizedJob }>("/webhook/sa-job-duplicate", {
      ...body,
      requesterEmail: session.user.email
    });
